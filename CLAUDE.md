...
**Message flow:** Transport receives message → constructs `IncomingMessage` → publishes `MessageIncoming` to EventBus → `MessageHandler.handle()` in `core/pipeline.py` → rate limit check → command dispatch (`core/commands.py`) → auth check → load context from SQLite → build system prompt with tools filtered by user permission + memory context → `ToolExecutor.run()` in `core/tool_executor.py` (LLM + tool-use loop, up to 10 iterations) → store response → publish `MessageOutgoing` with `OutgoingMessage` → Transport delivers with chunking.
...
- `core/commands.py` — `CommandHandler`: slash command dispatch (/forget, /context, /summarize, /jobs, /sudo, /memory, /pause, /resume, /status, /help)
...
- `planner/` — `models.py` (PlanStep, Plan), `engine.py` (PlanEngine: LLM-driven plan creation, step execution, failure handling, SQLite persistence)
...
**Webhooks:** `WebhookServer` (aiohttp) listens for external events. Built-in parsers for GitHub (push, PR, issues, workflow_run with `X-Hub-Signature-256` HMAC), Sentry (`sentry-hook-signature`), and generic (`X-Webhook-Secret`). Events normalized to `WebhookEvent` and published to the bus.

**Planner:** `PlanEngine` decomposes complex goals into 2-8 steps via LLM, executes sequentially with tool calls, handles failures (retry/skip/abort via LLM decision), caps at 15 tool invocations, persists plans in SQLite. Exposed as the `plan` tool (OPERATOR).
...
**New webhook endpoint:** Add a `WebhookEndpointConfig` entry to the `webhooks.endpoints` list in config. For custom providers, add a normalization function in `webhooks/handlers.py` and a validation function, then wire them into `WebhookServer._validate()` and `._normalize()`.

**New slash command:** Add an `elif command == "/yourcommand"` branch in `CommandHandler.handle()` in `core/commands.py`. Update the `/help` text.
